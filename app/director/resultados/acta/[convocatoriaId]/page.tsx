...
export default async function ActaPage({
  ...
  const { data: conv } = await admin
    .from("convocatorias")
    .select("id, nombre, fecha_examen, sede, estado, grados")
  ...
  const aspirantes = await Promise.all(
    (solicitudes || []).map(async (sol: any) => {
      const { data: res } = await admin
        .from("resultados")
        .select("bloque, calificacion, componente, comentarios")
        .eq("solicitud_id", sol.id);

      const resList = res || [];
      const comun    = resList.find((r) => r.bloque === "comun");
      const especif  = resList.find((r) => r.bloque === "especifico");
      ...
      return {
        ...
        comentarios:    [comun?.comentarios, especif?.comentarios].filter(Boolean).join(" / ") || "—",
      };
    })
  );
  ...
  const fecha   = conv.fecha_examen
    ? new Date(conv.fecha_examen + "T00:00:00").toLocaleDateString("es-ES", { day: "numeric", month: "long", year: "numeric" })
    : "—";
    ...
    <div className="mx-auto max-w-5xl">
      ...
      <div className="bg-white rounded-lg border border-[#54585B]/20 p-8 print:border-0 print:p-0">
        ...
        <div className="text-center border-b-2 border-[#7A1F2A] pb-6 mb-8">
          ...
          <h1 className="text-2xl font-bold text-[#191C1D] mb-1" style={{ fontFamily: "Montserrat, sans-serif" }}>
        ...
        <section className="grid grid-cols-2 gap-6 mb-8">
          ...
          <div>
            ...
            <span className={`inline-flex items-center rounded border px-2.5 py-0.5 text-xs font-bold uppercase ${
              isFinalizada
        ...
        <section className="mb-8">
          <h2 className="text-sm font-bold uppercase tracking-wide text-[#54585B] mb-3 border-b border-[#54585B]/15 pb-1">
            Resultados de los aspirantes ({aspirantes.length} presentados)
          </h2>
          ...
          <div className="grid grid-cols-2 gap-16">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-[#54585B] mt-1">
                {jueces.find((j) => j.rol === "juez")?.jueces?.nombre ?? "—"}
                {" "}
                {jueces.find((j) => j.rol === "juez")?.jueces?.apellidos ?? ""}
              </p>
            </div>
          </div>
